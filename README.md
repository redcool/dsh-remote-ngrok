...
| 手机报 **`Failed to load plugins` / `bundle script ... failed to load`** | ①（v13 已修）0.1.5 的 PDF.js bundle 用 ES2024 `Iterator`，iOS<17.4 无此全局 → 解析失败；②（v14 已修）index 被浏览器/边缘缓存 → dsh 重启/升级后 bundle **指纹 rev 变了**，手机一直请求旧 rev → 上游回 404/238B 错误文本（非 JS）→ loader 报错 | **立即**：手机 Safari 清站点缓存/无痕/强制刷新（丢弃旧 index）。**根治**：v14 起 proxy 对 index 强制 `cache-control: no-store`（每次访问拿当前 rev）；静态 bundle 保留 `immutable` 缓存（rev 内容寻址天然安全） |
...
- **v14（2026-09-23）**：修复手机偶发 `Failed to load plugins`——**bundle 指纹（rev）过期**。dsh 前端按内容指纹 rev 内容寻址（index 里 `/plugins/??...&rev=<hash>`）：换票 303 后浏览器落在干净 `/`，而 index 无 cache-control → Safari 启发式缓存旧 index → dsh 重启/升级后 rev 变化（实测旧 rev 请求上游回 238B 文本/404，非 JS → loader 报 "bundle script ... failed to load"）。修复：**html 分支强制回 `cache-control: no-store`**，每次访问都拿当前 index/当前 rev；静态资源保留上游 `immutable` 缓存（rev 寻址天然安全）。根因排查：当前 rev 的 12.3MB 合包经隧道 gzip→4.7MB/5s 正常，代理与隧道均未破坏；唯一变量是 rev 过期。
...
| 8 | **0.1.5 新增** `dsh-client-ui-sidebar-documentpreview`（PDF.js）bundle 顶层引用 **ES2024 `Iterator` 全局**（`typeof Iterator.prototype.join !== 'function'`（这类检测在 iOS<17.4 上 ReferenceError → 插件 import 失败） | proxy polyfill 追加 **Iterator shim**（构造 + `Iterator.prototype.join`/`[Symbol.iterator]`，挂到生成器与 Map/Set/Array 迭代器原型）+ **URL.parse**/**Promise.try**（均为 typeof 守卫、幂等注入） | 前端又引入更新的 ES 全局 → 扩 polyfill 即可 |
| 9 | 前端 bundle 按**内容指纹 rev 内容寻址**：index 引用 `/plugins/??...&rev=<hash>`，dsh 重启/升级后 rev 变化，旧 index 引用旧 rev → 404/错误文本 | `server.js` html 分支强制回 **`cache-control: no-store`**（每次都拿当前 index/当前 rev）；静态 bundle 保留上游 `immutable` 缓存（rev 寻址安全） | 若 dsh 改用固定 URL（无 rev 寻址）可去掉 no-store；若 rev 变为进程内随机（每次重启都变）则 no-store 是必须项 |），且用 `URL.parse` / `Promise.try`；iOS < 17.4 无 `Iterator`、iOS < 17.4 无 `URL.parse`、Safari < 18 无 `Promise.try`，缺失时手机报 `Failed to load plugins` | v13 polyfill：注入 `Iterator`（`join` + `[Symbol.iterator]`，含生成器/Map/Set/Array 迭代器原型挂载）、`URL.parse`、`Promise.try` | 前端又引用更新的 ES 提案 API（如 `Object.groupBy`、`Array.fromAsync`、`RegExp.escape`）→ 按同样模式扩 polyfill（注意与语法差异不同：polyfill 只救 API 缺失，救不了新语法解析失败） |
